...
提出了新的线性和非线性校准方法（基于LM算法），用于自校准旋转和变焦相机。 他们的方法基于无限单应性约束，并将$f，C_x，C_y$和$k_1$视为变量参数。 为了提高相机参数的准确性，他们使用了最大似然估计（MLE）和最大后验估计（MAP）。 他们通过保持原理点不变而更准确的结果。 他们还分析了径向畸变对自校准的影响，并使用多项式拟合对径向畸变的第一系数的变化进行了建模。 
